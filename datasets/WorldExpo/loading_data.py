...
import torch
import random
from datasets.WorldExpo.dataset_config import config

def loading_data(batch_size, sub_test='104207'):
    data_path = config.data_path

    main_transform = transforms.Compose([
        transforms.HSV(),
        transforms.RandomCrop(low_ratio=2),
    ...
    test_set = CCLoader(osp.join(data_path, 'test', sub_test), 'test', main_transform=val_main_transform, img_transform=img_transform, downscale=4, default_dis=config.padm_default_distance)
