...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        self.reg_conv1 = nn.Sequential(nn.Conv2d(input_channels, 128, kernel_size=3, padding=1), nn.ReLU(inplace=True)) 
        self.density_layer = nn.Sequential(nn.Conv2d(128, 1, 1), nn.ReLU(inplace=True))

        # self.fc = nn.Linear(output_channels, num_classes)

    def _forward_impl(self, x: Tensor, is_feature: bool) -> Tensor:
        x = self.conv1(x)   # 2
        f0 = x
        x = self.stage2(x)  # 4
        f1 = x
        ...
        x = self.stage4(x)  # 16
        f3 = x
        x = F.upsample_bilinear(x, scale_factor=2) # 8
        x = self.reg_conv1(x)
        reg_f = x
        out = self.density_layer(x)
    ...
    def forward(self, x: Tensor, is_feature: bool=False) -> Tensor:
